...
public class DoubleHashingImpl<K, V> implements IHashDouble, IHash<K, V> {
	...
	public void put(K key, V value) {
		...
		int i = 0;
		for (; flag; i++) {
			hash_func = (((Integer)key % slots) + i
					*(prime - ((Integer)key  % prime))) % slots;
			if (i == this.slots) {
				rehash = true;
				flag = false;
				collisions += 1;
	...
	public String get(K key) {
		boolean flag = true;
		for (int i = 0; flag; i++) {
			hash_func = ((key.hashCode() % slots) + i
					* (prime - (key.hashCode() % prime)))
					% slots;
	...
	public void rehash(K key, V value) {
		...
		for (IPair p : hash_table) {
			if (p != null) {
				...
				int i = 0;
				for (; flag; i++) {
					hash_func = (((Integer)p.getKey()  % slots) + i 
							* (prime - ((Integer)p.getKey() % prime))) % slots;
